...
export function OcrQuotaProgress({ usage, colorMode, labels }: OcrQuotaProgressProps) {
  ...
  const totalPercent = Math.min(100, Math.round((totalUsed / Math.max(1, totalLimit)) * 100))
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-[2px] ${colorMode === 'Dark' ? 'text-slate-400' : 'text-slate-500'}`}>
            <Gauge className="h-4 w-4" /> {labels?.ocrQuotaLabel || 'OCR quota'}
          </p>
          <p className="mt-1 text-lg font-black tabular-nums">{totalUsed} / {totalLimit}</p>
        </div>
        <div className={`rounded-2xl px-3 py-2 text-[10px] font-black uppercase ${totalPercent >= 90 ? 'bg-rose-50 text-rose-600' : totalPercent >= 70 ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>
          {totalPercent}%
        </div>
      </div>
      <div className={`mt-4 h-2 overflow-hidden rounded-full ${colorMode === 'Dark' ? 'bg-slate-800' : 'bg-slate-100'}`}>
        <div className={`h-full rounded-full transition-all ${totalPercent >= 90 ? 'bg-rose-500' : totalPercent >= 70 ? 'bg-amber-500' : 'bg-emerald-500'}`} style={{ width: `${totalPercent}%` }} />
      </div>
